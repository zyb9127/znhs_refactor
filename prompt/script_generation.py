...
常量分段（拼接顺序见 engine.prompt_builder.build_prompt 新格式分支）：
  ...
  SCRIPT_GEN_RULES        生成规则（防编造 / 防串填 / 槽位填充 / 模板严格保留）
...
_RULE3_TAIL = (
    ...
    "若该行事实包含多个指标（如历史用量、用户标签），仍须按模板中的同名槽位原样填入，"
    "不得自行提炼、摘要、扩写或把其他指标拼接进该槽位；不得因内容多而删除该槽位。\n"
)
_RULE4_HEAD = (
...
# 个性化约束（当上下文含用户标签/画像/性格类信息时由 build_prompt 自动追加）
SCRIPT_PERSONA_RULE = (
    "个性化约束：用户标签/画像/性格信息仅用于核对【上下文数据】中的已有事实，"
    "不得据此调整【话术模板】的文字、句式、句序、卖点顺序或信息范围，"
    "不得把标签名或画像字段名原样写进话术。"
)
