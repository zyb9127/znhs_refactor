...
常量分段（拼接顺序见 engine.prompt_builder.build_prompt 新格式分支）：
  ...
  SCRIPT_GEN_RULES        生成规则（防编造 / 防串填 / 槽位填充 / 模板为内容蓝本+口语泛化）
...
SCRIPT_MISSING_FACTS_TAIL_PASSTHROUGH = (
    ...
    "——尤其不得用当前套餐或推荐套餐的包含量（套餐流量、语音额度、月费）"
    "冒充用户的历史使用量（月均流量、主叫时长、月均消费）；"
    "但必须保留这些槽位对应的信息点，把槽位本身写成 <PH>"
    "（保留该信息点、不得删除），由坐席对客时口头补充。"
)
...
_RULE2_PASSTHROUGH = (
    ...
    "由坐席对客时口头补充；不得因取不到值而丢掉该信息点，也不得编造数值顶替"
    "（可正常口语化表达，但须保留 <PH> 供坐席补充）。"
...
_RULE4_HEAD = (
    "4. 以【话术模板】为内容蓝本、用你自己的自然口语重新表达：模板界定了本次话术要覆盖的"
    "环节、卖点/信息点、事实与大致推进顺序——这些环节与要点必须逐一覆盖、不遗漏也不新增"
    "（模板未提到的优惠、权益、活动、办理方式一律不得自行补充）；"
    "但不要逐字复述模板，请在覆盖全部要点的前提下更换措辞、句式、连接词与语气，"
    "并可在不打乱整体推荐逻辑的前提下适度调整句子顺序，"
    "使同一模板每次生成在表达上自然不同、更贴合当前用户——"
    "槽位事实值（尤其是数字、金额、套餐名与方括号内的数值）须严格照第 2、3 条原样填入、不得改动，"
    "可变的只是这些事实之外的行文表达。"
)
_RULE4_EMPTY_DROP = "只有第 2 条要求略过空值/零值时，才删除模板中对应的那句话。"
_RULE4_EMPTY_PASSTHROUGH = (
    "不得因槽位取不到值、或值为 0 而丢掉模板里对应的环节/信息点"
    "（按第 2 条填 <PH> 或照实填 0 即可）；"
    "只有【话术要求】明确要求不说的内容，才略过模板中对应的环节/语句。"
)
_RULE4_TAIL = (
